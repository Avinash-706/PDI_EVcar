...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // File upload preview (removed carPhoto as Step 3 Expert Details is deleted)
    ...
    // Setup Stepney Tyre conditional logic
    setupStepneyConditional();
...
function validateStep(step) {
    ...
    // Special validation for Step 5 (Exterior Body with 52 fields)
    if (step === 5) {
        ...
        for (let group of checkboxGroups) {
            ...
            // Check if image/remark is required (non-OK option selected)
            ...
            if (!isOkChecked) {
                ...
                if (hasNonOkChecked) {
                    // Special handling for match_all_glasses_serial_number - validate remark instead of image
                    if (groupName === 'match_all_glasses_serial_number') {
                        const remarkContainer = document.getElementById(groupName + '_remark_container');
                        if (remarkContainer && remarkContainer.style.display !== 'none') {
                            const textarea = remarkContainer.querySelector('textarea');
                            if (textarea && !textarea.value.trim()) {
                                ...
                                // Scroll to the field
                                remarkContainer.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                return false;
                            }
                        }
                    } else {
                        // Standard image validation for other fields
    ...
    // Special validation for Step 7 (Body Frame Accidental Checklist)
    // Images are ALWAYS mandatory, remarks are optional
    if (step === 7) {
        // Find all checkbox groups with data-step7-group attribute
        const checkboxGroups = currentStepElement.querySelectorAll('[data-step7-group]');
        ...
        // Validate all required fields (including images) using standard validation
        // Images are always required in Step 7 (set in HTML with required attribute)
        ...
        for (let field of requiredFields) {
            if (field.type === 'file') {
                ...
                if (!hasSavedFile && !hasNewFile) {
                    ...
                    const labelText = fieldLabel ? fieldLabel.textContent.replace('*', '').trim() : field.name;
...
function setupOkCheckboxLogic() {
    ...
    checkboxGroups.forEach(group => {
        ...
        // Check if this is a Step 7 group (images always mandatory, remarks optional)
        const isStep7Group = group.hasAttribute('data-step7-group');
        
        // Special handling for match_all_glasses_serial_number - use remark instead of image
        const isGlassesField = groupName === 'match_all_glasses_serial_number';
        
        // Function to update container visibility
        function updateContainerVisibility() {
            // Check if OK/Matching is checked
            ...
            if (isStep7Group) {
                // STEP 7: ONLY handle remark visibility (image is ALWAYS visible in HTML)
                const remarkContainer = document.getElementById(groupName + '_remark_container');
                
                // Remark field: show only if non-OK option selected, always optional
                if (remarkContainer) {
                    if (hasNonOkChecked && !isOkChecked) {
                        remarkContainer.style.display = 'block';
                    } else {
                        remarkContainer.style.display = 'none';
                        // Clear remark value when hiding
                        const remarkInput = remarkContainer.querySelector('input[type="text"]');
                        if (remarkInput) {
                            remarkInput.value = '';
                        }
                    }
                }
            } else if (isGlassesField) {
                // Step 5: match_all_glasses_serial_number - use remark instead of image
                const remarkContainer = document.getElementById(groupName + '_remark_container');
                ...
                // Step 5: Other exterior body fields - conditional image
                const imageContainer = document.getElementById(groupName + '_image_container');
        ...
        checkboxes.forEach(checkbox => {
            checkbox.addEventListener('change', function() {
                ...
                // Update container visibility after checkbox change
    ...
    .then(data => {
        ...
        if (data.success && data.draft_data) {
            ...
            setTimeout(() => {
                ...
                // Trigger OK checkbox logic for Step 5 (Exterior Body) and Step 7 (Body Frame)
                // This ensures image/remark containers show/hide correctly based on saved checkbox values
                const checkboxGroups = document.querySelectorAll('[data-ok-group]');
                checkboxGroups.forEach(group => {
                    const groupName = group.getAttribute('data-ok-group');
                    const okCheckbox = group.querySelector('[data-ok-checkbox]');
                    ...
                    // Check if this is a Step 7 group
                    const isStep7Group = group.hasAttribute('data-step7-group');
                    ...
                    if (isStep7Group) {
                        // STEP 7: Only handle remark container (image is always visible in HTML)
                        const remarkContainer = document.getElementById(groupName + '_remark_container');
                        if (remarkContainer) {
                            if (hasNonOkChecked && !isOkChecked) {
                                remarkContainer.style.display = 'block';
                ...
                // Trigger Tool Kit conditional logic for Step 11
                ...
                // Trigger Stepney Tyre conditional logic for Step 13
                // This ensures stepney fields show/hide correctly based on saved value
                const stepneyRadios = document.querySelectorAll('input[name="stepney_tyre_depth_check"]');
                const stepneyTreadDepthContainer = document.getElementById('stepney_tread_depth_container');
                const stepneyDateContainer = document.getElementById('stepney_date_container');
                const stepneyShapeContainer = document.getElementById('stepney_shape_container');
                const stepneyTreadDepthInput = document.getElementById('stepneyTyreTreadDepth');
                const stepneyShapeInputs = document.querySelectorAll('input[name="stepney_tyre_shape"]');
                
                if (stepneyRadios.length) {
                    const selectedValue = document.querySelector('input[name="stepney_tyre_depth_check"]:checked')?.value;
                    
                    if (selectedValue === 'Stepney Not Available') {
                        // Hide the three dependent fields
                        if (stepneyTreadDepthContainer) stepneyTreadDepthContainer.style.display = 'none';
                        if (stepneyDateContainer) stepneyDateContainer.style.display = 'none';
                        if (stepneyShapeContainer) stepneyShapeContainer.style.display = 'none';
                        
                        // Remove required attribute
                        if (stepneyTreadDepthInput) {
                            stepneyTreadDepthInput.removeAttribute('required');
                        }
                        
                        stepneyShapeInputs.forEach(input => {
                            input.removeAttribute('required');
                        });
                    } else {
                        // Show the three dependent fields
                        if (stepneyTreadDepthContainer) stepneyTreadDepthContainer.style.display = 'block';
                        if (stepneyDateContainer) stepneyDateContainer.style.display = 'block';
                        if (stepneyShapeContainer) stepneyShapeContainer.style.display = 'block';
                        
                        // Add required attribute back (unless file already uploaded)
                        if (stepneyTreadDepthInput && !stepneyTreadDepthInput.dataset.savedFile) {
                            stepneyTreadDepthInput.setAttribute('required', 'required');
                        }
                        
                        // Set first radio button as required
                        if (stepneyShapeInputs.length > 0) {
                            stepneyShapeInputs[0].setAttribute('required', 'required');
                        }
                    }
                }
            }, 100);
...
// Setup Stepney Tyre conditional display
function setupStepneyConditional() {
    const stepneyRadios = document.querySelectorAll('input[name="stepney_tyre_depth_check"]');
    const stepneyTreadDepthContainer = document.getElementById('stepney_tread_depth_container');
    const stepneyDateContainer = document.getElementById('stepney_date_container');
    const stepneyShapeContainer = document.getElementById('stepney_shape_container');
    ...
    if (!stepneyRadios.length) return;
    
    function toggleStepneyFields() {
        const selectedValue = document.querySelector('input[name="stepney_tyre_depth_check"]:checked')?.value;
        ...
        if (selectedValue === 'Stepney Not Available') {
            ...
            if (stepneyTreadDepthInput) {
                stepneyTreadDepthInput.removeAttribute('required');
                stepneyTreadDepthInput.value = ''; // Clear the file input
            }
            
            stepneyShapeInputs.forEach(input => {
                input.removeAttribute('required');
                // DON'T uncheck - preserve the value for when it's shown again
            });
        } else {
            // Show the three dependent fields
            if (stepneyTreadDepthContainer) stepneyTreadDepthContainer.style.display = 'block';
    ...
    // Add event listeners
    stepneyRadios.forEach(radio => {
        radio.addEventListener('change', toggleStepneyFields);
    });
